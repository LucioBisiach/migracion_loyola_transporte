...
services = sock.execute(dbname,uid,pwd,'service.services','search', [('name', '!=', ''), ('outsourced_service','=',False)])

for service in services:
    service_data = sock.execute(dbname,uid,pwd,'service.services','read',service,['id', 'name', 'label_service','date_start','date_stop','n_ref', 'customer', 'employee'])
    ...
    # Empleados
    for empleado in service_data[0]['employee']:
        if empleado is not int:
            print(empleado)
    # print(service_data[0]['employee'])
